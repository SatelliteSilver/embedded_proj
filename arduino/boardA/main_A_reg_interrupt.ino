...
void moveAtSpeed(int speed) {
  ...
  int corrected_right = -speed * 0.8;   // 예시 보정
...
bool rotateToAbsAngle(float targetAbsAngle) {
  ...
  while (true) {
    ...
    // 회전 중에는 필터 미적용 즉시 yaw 사용
    float rawYaw = stcAngle.Yaw;
    if (rawYaw < 0) rawYaw += 360.0f;
    float heading_now = wrap360(rawYaw);
...
void loop() {
  ...
  while (rxQueue.getLength() >= 4) {
    ...
    uint32_t packet = ((uint32_t)r1 << 24) |
                      ((uint32_t)r2 << 16) |
                      ((uint32_t)r3 << 8)  |
                      r4;

    uint8_t rx_crc   = packet & 0xFF;
    ...
    uint8_t rx_seqbit = (packet >> 8) & 0x1; (void)rx_seqbit;
    blinkOnce();
    ...
    if (speed_type == 0x01) new_v = speed_val;

    if (angle_type == 0x02) {
      if (angle_val & 0x80) new_a = - (float)(angle_val & 0x7F);
      else                  new_a =   (float) angle_val;
    }

    // C) 상태 머신 트리거
    if (new_v > 0 || new_a != 0.0f) {
      targetSpeed = constrain(new_v, 0, MAX_MOVE_PWM);
      targetAngle = wrap360(currentHeading + new_a); // 상대각
      setMotorSpeedDifferential(0, 0);
      moveAtSpeed(0);
  ...
  // 9) 20ms 틱 대기 (delay 대체)
  while (!g_tick20ms) { /* wait for ISR */ }
  g_tick20ms = 0;
}
