...
// setup() 함수 위에 아래 함수를 추가하세요.

//경로 이동 완료후 다음 경로 송신을 요구하는 함수 
void sendStatusPacket() {
  // status=1 에 해당하는 32비트 패킷 생성 (9번째 비트를 1로 설정)
  // 1UL << 9; 는 32비트 정수 1을 왼쪽으로 9번 비트 이동시키는 연산입니다.
  uint32_t status_packet = (1UL << 9);

  // PC가 보내는 방식(MSB first)과 동일하게 4바이트로 분해
  uint8_t b1 = (status_packet >> 24) & 0xFF; // MSB
  uint8_t b2 = (status_packet >> 16) & 0xFF;
  uint8_t b3 = (status_packet >> 8)  & 0xFF;
  uint8_t b4 = status_packet & 0xFF;         // LSB
  ...
  if (DEBUG) {
    Serial.println("[TX_STATUS] Sent status=1 packet to PC.");
  }
}
// ===== Arduino 기본 Setup =====
// ===== Arduino 기본 Setup =====
    ...
    if (new_v > 0 || new_a != 0.0f) {
        ...
        if (abs(err) <= ANGLE_DEADBAND) { // 회전 생략
            if (targetSpeed > 0) {
                ...
                // sendHeadingFrame(); // (삭제)
                sendStatusPacket(); // "회전할 필요 없어서 바로 완료!" 신호 전송
                currentState = IDLE;
            }
        } else { // 회전 시작
      ...
      if (newCommandReceived) { // 새 회전 명령 들어왔으면
        ...
        if (rotateToAbsAngle(targetAngle)) { // 회전 성공
          if (targetSpeed != 0) { // 직진 속도 있으면 직진 시작
            ...
            // sendHeadingFrame(); // [삭제]
            sendStatusPacket(); // "회전만 하고 완료!" 신호 전송
            currentState = IDLE;
          }
        } else { // 회전 실패 (타임아웃)
          // Serial.print 사용 시 충돌 가능성 있음!
