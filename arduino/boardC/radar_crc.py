...
# ----- CRC-8 (Arduino A와 동일) -----
CRC8_POLY   = 0x07  # x^8 + x^2 + x + 1
CRC8_INIT   = 0x00
CRC8_XOROUT = 0x00

def crc8_24(upper24: int) -> int:
    """
    upper24: 패킷의 상위 24비트 (31..8)만 넣어서 CRC-8 계산
    """
    crc = CRC8_INIT
    for i in (2, 1, 0):  # MSB 먼저
        b = (upper24 >> (i * 8)) & 0xFF
        crc ^= b
        for _ in range(8):
            if crc & 0x80:
...
# --- 시리얼 설정 (비블로킹) ---
ser_radar = serial.Serial('COM10', 115200, timeout=0)  # ← 0: non-blocking
ser_bt    = serial.Serial('COM12',  9600, timeout=0, write_timeout=0.2)
...
def build_packet(speed, angle, status, toggle_seq=True):
    global TX_SEQBIT
    ...
    # --- 상위 24비트(31..8) 먼저 구성 ---
    pkt = 0
    pkt |= (0b01 << 30)                 # speed type
    pkt |= (speed & 0xFF) << 22         # speed val
    pkt |= (0b10 << 20)                 # angle type
    pkt |= (angle_val << 12)            # angle val
    pkt |= (0b11 << 10)                 # status/mode type
    pkt |= (status & 0x1) << 9          # bit9 = status

    # --- bit8: 우리가 말한 “나머지 1비트” ---
    if toggle_seq:
        TX_SEQBIT ^= 1                  # 0→1→0→1 토글
    pkt |= (TX_SEQBIT & 0x1) << 8       # bit8에 삽입

    # --- 하위 8비트: CRC-8(상위 24비트 기준) ---
    upper24 = pkt >> 8                  # 31..8
    crc = crc8_24(upper24)
    pkt |= crc                          # 7..0
...
def read_bt_binary():
    if ser_bt.in_waiting >= 4:
        ...
        rx_crc   = val & 0xFF          # 7..0
        calc_crc = crc8_24(val >> 8)   # 31..8
        rx_seq   = (val >> 8) & 0x1    # A가 STATUS 보낼 때 토글하는 1비트
        status   = (val >> 9) & 0x1

        # A처럼: CRC가 0이면 “구버전”으로 보고 그냥 통과
        if rx_crc != 0x00 and rx_crc != calc_crc:
            print(f"[BT RX] CRC mismatch: got=0x{rx_crc:02X}, want=0x{calc_crc:02X}, val=0x{val:08X}")
            return None

        # 디버깅용으로 시퀀스도 보고 싶으면:
        # print(f"[BT RX] status={status}, seq={rx_seq}")
...
def do_attack():
    ...
    if block_idx is not None:
        ...
        # 1. CAR에서 AVOID 지점까지의 중간 지점을 계산
        mid_x = (x1 + ax) / 2
        mid_y = (y1 + ay) / 2

        # 2. 경로를 3개 세그먼트로 구성
        path.append((mid_x, mid_y)) # SEG1 종료점
        path.append((ax, ay))       # SEG2 종료점
        path.append((x2, y2))       # SEG3 종료점
        
        print(f"[PATH] AVOID via MID ({mid_x:.1f}, {mid_y:.1f}), AVOID ({ax:.1f}, {ay:.1f}) around BLOCK ({bx:.1f},{by:.1f})")
    ...
    for i, seg in enumerate(segments):
        # 첫 번째 세그먼트는 절대 각도 기준, 이후 세그먼트는 이전 세그먼트 대비 상대 각도 기준
    ...
    # 1. PC의 BT 수신 버퍼를 완전히 비웁니다. 
    #  (첫 번째 응답을 깨끗하게 받기 위해 필수)
    try:
        ser_bt.reset_input_buffer() 
    except Exception as e:
        print(f"[WARN] Failed to reset BT buffer on start: {e}")

    # --- 첫 번째 세그먼트 송신 (SEG0) ---
    if len(segments) == 0:
        info("No segments to run.")
        return
    ...
    try:
        ...
        info(f"Transmission failed (SEG0): {e}")
        waiting_ok = False
        return

    # --- 첫 번째 세그먼트 완료 대기 (Completion of SEG0) ---
    # 이 대기에서 실패하고 타임아웃이 발생하고 있습니다.
    t0 = time.time()
    next_ok = False
    print(f"[BT WAIT] Waiting for STATUS=1 after sending SEG0...") # <--- 추가된 디버깅 라인

    while time.time() - t0 < 10.0: # 10초 타임아웃
        status_reply = read_bt_binary()

        # --- ▼▼▼ 디버깅 출력 추가 ▼▼▼ ---
        if status_reply is not None:
            print(f"[BT RX DEBUG] Received status: {status_reply} (Waiting for 1)")
        # --- ▲▲▲ 디버깅 출력 추가 ▲▲▲ ---

        if status_reply == 1:
            next_ok = True
            print("[BT RX] status=1 (SEG0 Complete) → TX SEG1")
            break
        time.sleep(0.05)

    if not next_ok:
        info("No status=1 response for SEG0 (Operation Aborted).")
        print("[BT TIMEOUT] Did not receive STATUS=1 for SEG0 within 10 seconds.") # <--- 추가된 디버깅 라인
        waiting_ok = False
        return # 여기서 타임아웃 시 함수 종료
    
    # --- 이후 나머지 세그먼트 전송 루프 (SEG1부터 끝까지) ---
    for i, seg in enumerate(segments[1:], start=1):
        
        # 이전 세그먼트(SEG i-1)가 완료되었으니, 버퍼를 비울 필요 없이 다음 명령 전송
        # (만약 여기서 또 STATUS=1 응답을 기다린다면 ser_bt.reset_input_buffer()가 필요함)

        # 2. 다음 세그먼트 전송 (TX SEG i)
        ...
        try:
            ...
            info(f"Transmission failed (SEG{i}): {e}")
            waiting_ok = False
            return
            
        # 3. 다음 세그먼트 완료 대기 (Completion of SEG i)
        t0 = time.time()
        next_ok = False
        
        while time.time() - t0 < 10.0: # 10초 타임아웃 루프
            status_reply = read_bt_binary()
            if status_reply == 1:
                next_ok = True
                print(f"[BT RX] status=1 (SEG{i} Complete) → TX SEG{i+1}")
                break
            time.sleep(0.05)
        
        if not next_ok:
            info(f"No status=1 response for SEG{i} (Operation Aborted).")
            waiting_ok = False
            return
